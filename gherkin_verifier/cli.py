...
Uso:
  gherkin-verify features/01_contraditorias.feature
  gherkin-verify features/                          # todos os .feature
  gherkin-verify features/ --json                   # saída JSON
  gherkin-verify features/ --md                     # relatório completo em Markdown
  gherkin-verify features/ --verbose                # detalhes extras
...
def report_to_markdown(report: VerificationReport) -> str:
    ...
    # Pré-checagens simbólicas (SymPy) se disponíveis
    pre = getattr(report, "precheck", None)
    if isinstance(pre, dict) and pre.get("total_potentials", 0) > 0:
        lines.append("")
        lines.append("### Pré-checagens simbólicas")
        lines.append("")
        for item in pre.get("potential", []):
            lines.append(
                f"- {item.get('key', '')}: pos={item.get('pos', 0)}, neg={item.get('neg', 0)}"
            )
